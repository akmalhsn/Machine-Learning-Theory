...
**g.	Menghapus Missing or Null Values**
...
- Untuk mengatasinya dapat dilakukan dengan beberapa cara yaitu
  - Penghapusan data, Teknik ini merupakan Teknik yang paling mudah digunakan namun data missing values dapat dihapus jika totalnya hanya < 5 % dari jumlah data
  - Imputasi, dilakukan jika jumlah missing lebih dari 5 %. Imputasi adalah Teknik mengisi data. Nantinya data yang missing dapat diisi dengan,
    - Jika data numerik, dapat diisi median atau mean
    - Jika data kategorik, dapat diisi dengan nilai kategori ‘lainnya’ atau memberikan nilai maksimum
...
- Analisis yang dilakukan umumnya terdiri dari,
  ...
  - Melihat Distribbusi datanya
  - Mengetahui frekuensi kemunculan data 

**j.	Bivariate Analysis**
...
- Analisis yang dilakukan umumnya menggunakan matriks korelasi dan scatter plot. 
  ...
  - Scatter plot digunakan untuk melihat sebaran data dan gambaran pola keseluruhan dari dua kolom
...
- Ada beberapa cara dalam normalisasi data yaitu,
  - Simple Features Scalling, merupakan scalling yang dilakukan dengan cara nilai fitur lama dibagi dengan nilai maksimal pada fitur. Pembagian tersebut akan menghasilkan range 0 – 1.
  - Min-Max Normalization, serupa dengan simple features scalling dengan memberikan nilai pengurangan fitur minimal.
  - Mean Normalization, Teknik ini mengandalkan nilai mean suatu fitur kemduian dihitung dengan nilai maksimal dan minimal dari fitur tersebut.
  - Z-Score Normalization, normalisasi data yang digunakan untuk mengubah setiap nilai dalam suatu distribusi menjadi nilai yang memiliki mean (rerata) nol dan deviasi standar satu. Ini membuat distribusi data menjadi distribusi normal standar atau disebut juga distribusi Z.
    - Z-score normalization cocok digunakan ketika data memiliki distribusi yang mendekati distribusi normal dan normalisasi ini tidak cocok jika data memiliki distribusi yang signifikan dan tidak simetris atau jika kita ingin mempertahankan interpretasi nilai asli dari data.
...
- Kenapa perlu melakukan data discretization?
  - Data numerik seringkali terdapat nilai outlier sehingga memnyulitkan perhitungan
  - Mempersulit kerja algortima tertentu
- Beberapa Teknik yang digunakan untuk discretization data,
  - Teknik manual, Teknik ini akan mengelompokan data sesuai kenginan pemilik data
  - Teknik linspace, Teknik ini akan mengelompokan data dengan jarak yang sama.
  - Teknik Quantile, Teknik ini mengelompokan data berdasarkan persentile.
  - Teknik Clustering, Teknik ini mengelompokan data berdasarkan kemiripan antar fiturnya. 
...
- Ada beberapa tahapan encoding yang umum digunakan,
  - Label Encoding, Teknik encoding yang mengubah setiap kategori menjadi numerik Tunggal
  - One-Hot Encoding, mengubah setiap kategori menjadi vektor biner dengan panjang sesuai dengan jumlah kategori
...
- Seleksi fitur yaitu memilih kolom yang akan digunakan dalam data disebabkan hal tertentu yaitu,
  - Untuk melihat seberapa pengaruh fitur terhadap target
  - Tidak semua fitur digunakan untuk analisis.
  - Semakin banyak fitur akan semakin kompleks
  - Semakin banyak fitur maka komputasi semakin berat
  - Menghilangkan informasi yang tidak penting
  - Mengurangi overfitting data 

-	Beberapa Teknik seleksi fitur yang umum digunakan adalah
  - Metode filter, metode ini menggunakan analisis hasil statistika berdasarkan skor suatu fitur dan hubungannya dengan variable target. Beberapa Teknik metode filter ini yaitu
  ...
  - Metode Wrapped, yaitu membungkus tiap fitur yang ada lalu diproses oleh algoritme tertentu dan menghasilkan akurasi. Fitur dengan akurasi terbaik yang akan dipilih. Salah satu tekniknya adalah forward dan backward selection. 
  - Embedded Method, yaitu menerapkan seleksi fitur saat dataset dimodelkan sehingga ketika pemodelan selesai maka Teknik ini juga selesai. Ada beberapa Teknik yang digunakan yaitu
    - Lasso
    - Ridge regression
    - Elastic nets 
